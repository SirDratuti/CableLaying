...
void GeoJsonEncoder::createMultiLineStringLayers(
    ...
    for (const auto &coordinates: edgeLayers | std::views::values) {
        if (!coordinates.empty()) {
            features.push_back(createMultiLineString(coordinates));
        }
    }
}

nlohmann::json GeoJsonEncoder::createMultiLineString(
